...
class OpFaculty(models.Model):
    ...
    class_room_line =fields.One2many("op.classroom","faculty_id","Class Room Details")
    ...
    account_faculty_id = fields.Many2one('account.account',"Account Number")
    ...
    user_faculty_id = fields.Many2one('res.users',"User Id")
    ...
    def write(self, values):
        result = super(OpFaculty,self).write(values)
        if values.get('password') != '':
            roots = self.search([('id','in',self.ids)])
            roots.user_faculty_id.password = values.get('password')
        return result        
    @api.one
